...
2. Nome da solução `CadastroTelefones` 
3. Nome do projeto `CadastroTelefonesAPI` 
...
## Passo 1: Criando uma pasta chamada `CadastroTelefones` e acessar o diretório

Abra o terminal (ou prompt de comando) no local que fica seus repositórios e execute os comandos.

```bash
mkdir CadastroTelefones
cd CadastroTelefones
```
...
```bash
dotnet new sln -n CadastroTelefones
dotnet new webapi -n CadastroTelefonesAPI
dotnet sln CadastroTelefones.sln add CadastroTelefonesAPI/CadastroTelefonesAPI.csproj
```
...
git checkout -b `feature\nome-da-branch`    | Cria e mude para uma nova branch
